...
The migration tool is running as a job in Databricks. The state of each migration is persisted in a Cosmos NOSQL database which must be provisioned upfront. This NoSQL database contains collections which persists migration status, any errors as well as any documents which could not be migrated. </br>
