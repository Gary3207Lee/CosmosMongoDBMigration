...
The Spark Migration Utility is running as a job in Databricks. The state of each migration is persisted in a Cosmos NOSQL database which must be provisioned upfront. This NoSQL database contains collections which persists migration status, any errors as well as any documents which could not be migrated. </br>
...
The migration tool uses connection strings to connect the following resources.
  ...
  - Target Cosmos DB MongoDB account </br>

Intermediate Cosmos DB NoSQL account (only in case of Online Migrations)  
  - Status / Intermediate Cosmos DB NoSQL account </br>

Verify the connection strings by using appropriate utilities to establish connections.  

### Migration configuration

The migration tool uses a JSON based configuration file as input. A detailed explanation of each option and sample JSON configuration files can be found [here](/99_Practice/First_MIG.json) </br>
